...
void DescSort(int[ColSize], int i, int CSiz);
...
void DescSort(int iNumber[ColSize], int i, int CSize)
{
	int j;
	...
	for (i = 0; i < ColSize - 1; i++)
	{
		for (j = i + 1; j < ColSize; j++)
		{
			...
			{
				//Swap_Reference(iNumber[i],iNumber[j]);
				Swap_Address(&iNumber[i], &iNumber[j]);
