...
void selectionSort(int iNumber[ColSize], int i, int CSize);
int main(void){
	//입력-선언부
...
void selectionSort(int iNumber[ColSize], int i, int CSize)
{
	...
	for (i = 0; i < ColSize - 1; i++)
	{
		...
		{
			if (iNumber[indexMin] > iNumber[j])
			{
				indexMin = j;
			}
		}
		Swap_Reference(iNumber[indexMin], iNumber[i]);
	}
}
#endif